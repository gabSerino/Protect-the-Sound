...
public class EnemyAI_Brain : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        // Se era morto mentre caricava, gli resettiamo la velocit� base
    ...
    private void CheckAggro()
    {
        ...
        if (hitPlayers.Length > 0)
        {
            ...
            // CONTROLLO FONDAMENTALE: Il player esiste e NON � morto?
        ...
        // Se non ha trovato nessun player nel raggio, OPPURE se il player che ha trovato � MORTO
        if (!foundAlivePlayer)
        {
            // Abbandona l'inseguimento e torna "Idle" (cos� andr� a cercare le Casse)
            if (currentState == EnemyState.ChasingPlayer || currentState == EnemyState.AttackingPlayer || currentState == EnemyState.PreparingCharge)
            {
                ...
                if (stats != null && agent != null)
                {
                    ...
                    agent.speed = stats.moveSpeed; // Torna alla velocit� normale
                }
            }
        }
    }
    ...
    // Una cassa "target" � valida solo se esiste ancora ed � attiva (non distrutta)
